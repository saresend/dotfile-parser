...
use super::{ID, assignment::AttributeList};
...
impl Constructable for Node {
    ...
    fn from_lexer(mut token_stream: crate::lex::PeekableLexer) -> anyhow::Result<(Self, crate::lex::PeekableLexer), anyhow::Error> {
        if let Some(Token::ID) = token_stream.next() {
           ...
           return Ok((Self { id: node_id, attribute_list: agroup.0 }, agroup.1));
        }
...
mod tests {
    ...
    fn node_statement_sanity1_test() {
        ...
        let asgng_1 = vec![Assignment::new("color","blue"),
